...
# Behind nginx, trust X-Forwarded-Proto so build_absolute_uri() emits https://
# links (used for og:image/og:url in shared-link previews).
...
if not _database_url.startswith(("postgresql://", "postgres://")):
    raise Exception(
        f"DATABASE_URL must start with 'postgresql://' — got: {_database_url[:30]!r}"
    )
...
# Include Vite built assets if the folder exists
...
# Without this, catalog.* logger.info() calls (e.g. the MoySklad webhook
# handler) go nowhere — Python's fallback handler only shows WARNING+.
LOGGING = {
    "version": 1,
